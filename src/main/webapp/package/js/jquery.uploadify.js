...
(function($) {
	...
	var handlers = {
		...
		onSelect : function(file) {
			...
			var exts = ["jpg","png","gif","doc","docx","exe","dll","xls","xlsx","ppt","pptx","txt","zip","rar","7z","mp3","wav"];
			var flag = false;
			...
			if (ITC_Len(fileName) > 25) {
				fileName = ITC_Substr(fileName,0,25) + '...';
			}
...
function _removeFail(fileWrap){
	fileWrap.find(".itc_upload_delfail").css('display','block').click(function(){
		$(this).parents(".itc_upload_wrap").remove();
	});
	fileWrap.find(".itc_upload_cancel").remove();
}
...
(function($){
	$.fn.extend({
		ITCUI_FileUpload : function(options){
			...
			$("<span class='start_upload'>开始上传</span>").appendTo(_this).addClass("itc_link").css({
				"height":"30px",
				"line-height":"30px",
			...
			options.swf = options.swf || "../package/js/uploadify.swf";
			options.buttonText = options.buttonText || "选择文件";
			options.buttonClass = options.buttonClass || "itc_link";
			options.auto = options.auto || false;
			options.width = options.width || "72";
			options.itemTemplate = options.itemTemplate || '<div class="itc_upload_wrap bbox" fileID="${fileID}">' + 
				'<div class="itc_upload_icon icon_${fileExt} ml8">' +
				'</div>' +
				'<div class="itc_upload_detail_wrap ml4">' +
					'<div class="itc_upload_filename">' +
						'<span>${fileName}</span> <a onclick="$(\'#${instanceID}\').uploadify(\'cancel\',\'${fileID}\')" class="itc_upload_cancel itc_link fr">取消</a>' +
						'<a class="itc_upload_delfail itc_link fr" style="display:none">删除</a>' +
					'</div>' +
			...
			options.onUploadSuccess = options.onUploadSuccess || function(file, data, response){
				...
				fileWrap.find(".itc_upload_progess_wrap").remove();
				if(data.status==1){
					fileWrap.find(".itc_upload_progress_txt").append('<span class="itc_upload_finish">完成</span>');
					fileWrap.find(".itc_upload_cancel").remove();
					if(data.msg){
						fileWrap.find(".itc_upload_delfail").css('display','block').click(function(){
							Notice.confirm("确认删除|是否删除该文件？",function(){
								$.ajax({
									"url" : data.msg,
									"type" : "GET",
									"dataType" : "json",
									"error" : function(){
										Notice.errorTopNotice("文件删除失败");
									},
									"success" : function(data){
										if(!data || data.status==1){
											Notice.successNotice("文件删除成功");
											fileWrap.remove();
										}
										else{
											Notice.errorTopNotice("文件删除失败");
					...
					fileWrap.find(".itc_upload_progress_txt").append('<span class="itc_upload_fail">' + data.msg + '</span>');
					_removeFail(fileWrap);
				}
			};
			...
			options.onUploadError = options.onUploadError || function(file, errorCode, errorMsg, errorString){
				...
				var fileWrap = $("#" + _this[0].settings.queueID).children("[fileID='" + file.id + "']");
				fileWrap.find(".itc_upload_progess_wrap").remove();
				fileWrap.find(".itc_upload_progress_txt").append('<span class="itc_upload_fail">上传失败</span>');
				_removeFail(fileWrap);
			};
			...
			options.onUploadStart = function(file){
				...
				$("#" + _this[0].settings.queueID).children("[fileID='" + file.id + "']").find(".itc_upload_progress_txt").prepend(progressHtml);
			};
